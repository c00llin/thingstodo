...
export function AnytimeView() {
  ...
  return (
    <div className="mx-auto max-w-2xl p-6">
      <h2 className="mb-6 text-2xl font-bold text-neutral-900 dark:text-neutral-100">Anytime</h2>
      ...
      {data?.areas.map((areaGroup) => (
        <div key={areaGroup.area.id} className="mb-8">
          ...
          {areaGroup.standalone_tasks.length > 0 && (
            <TaskGroup
              title="Other"
              tasks={areaGroup.standalone_tasks}
              showProject={false}
            />
          )}
        </div>
      ...
      {data?.no_area && (
        <>
          ...
          {data.no_area.standalone_tasks.length > 0 && (
            <TaskGroup
              title="No Project"
              tasks={data.no_area.standalone_tasks}
              showProject={false}
            />
          )}
        </>
